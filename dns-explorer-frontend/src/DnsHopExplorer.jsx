import React, { useMemo, useState, useEffect, useRef } from "react";
  ...
  import { Activity, Server, Globe, Zap, AlertTriangle, RefreshCcw, Link, Database, ChevronLeft, ChevronRight, Clock, Network, CheckCircle, ArrowRight } from "lucide-react";
  ...
  const SAMPLE = {
    ...
    "summary":{"final_ips":["172.217.19.36","142.250.191.4"],"total_ms":221.73,"hops":3,"cache_saved_ms":0},
    "trace":[
      {"step":1,"server":"199.7.91.13","role":"ns","question":{"name":"www.google.com.","type":"A"},"answer":[],"additional":[],"authority":[{"name":"com.","rdtype":"NS","ttl":172800,"records":[{"value":"a.gtld-servers.net."}]}],"rtt_ms":49.52,"cached":false},
      {"step":2,"server":"192.5.6.30","role":"ns","question":{"name":"www.google.com.","type":"A"},"answer":[],"additional":[{"name":"ns1.google.com.","rdtype":"A","ttl":172800,"records":[{"value":"216.239.32.10"}]}],"authority":[{"name":"google.com.","rdtype":"NS","ttl":172800,"records":[{"value":"ns1.google.com."},{"value":"ns2.google.com."}]}],"rtt_ms":83.33,"cached":false},
      {"step":3,"server":"216.239.34.10","role":"ns","question":{"name":"www.google.com.","type":"A"},"answer":[{"name":"www.google.com.","rdtype":"A","ttl":300,"records":[{"value":"172.217.19.36"},{"value":"142.250.191.4"}]}],"additional":[],"authority":[],"rtt_ms":85.58,"cached":false}
    ],
    "cname_chain":[]
  };
  ...
  function detectHopLabel(serverIp, hop) {
    ...
    const hasTldNS = (hop?.authority || []).some(a => a.name && /^[a-z0-9-]+\.$/i.test(a.name));
  ...
  function summarizeHop(hop) {
      ...
      if (answers.length > 0) {
        if (ips.length > 0) {
          return `Authoritative answer from ${serverInfo} - resolved to ${ips.length} A record${ips.length > 1 ? 's' : ''}: ${ips.slice(0,2).join(', ')}${ips.length > 2 ? '...' : ''} (${hop.rtt_ms}ms)`;
        ...
        if (nsTargets.length > 0) {
          ...
          return `NS referral from ${serverInfo} - delegated ${zone} to ${nsTargets.length} nameserver${nsTargets.length > 1 ? 's' : ''} (${hop.rtt_ms}ms)`;
        } else {
          return `No response from ${serverInfo} - possible NXDOMAIN or timeout (${hop.rtt_ms || 'timeout'}ms)`;
        }
      }
  }
  ...
  // Enhanced DNS Resolution Animation with professional design
  function DnsResolutionAnimation({ hops, isAnimating, onNodeHover, hoveredNode, onStepChange, onResolved }) {
    ...
    const getServerPosition = (step) => {
      const totalSteps = Math.max(hops.length, 3);
      const width = 600;
      const startX = 150;
      const spacing = (width - 100) / (totalSteps - 1);
      return { 
        x: startX + step * spacing, 
        y: 180 
      };
    };
  
    const getIPPosition = (index, total) => {
      if (total === 1) return { x: 750, y: 180 };
      const spacing = 50;
      const startY = 180 - ((total - 1) * spacing) / 2;
      return { x: 750, y: startY + index * spacing };
    };
  
    const getRoleColor = (role) => {
      switch (role) {
        case "root": return "#3b82f6";
        case "tld": return "#8b5cf6";
    ...
    const getRoleIcon = (role) => {
      switch (role) {
        ...
        return (
        <div className="relative w-full min-w-[768px] md:min-w-0 bg-gradient-to-br from-gray-900 via-black to-gray-900 border border-gray-700 rounded-xl overflow-hidden">
        ...
        <div className="relative p-8" style={{ minHeight: "400px" }}>
          <svg width="100%" height="100%" className="absolute inset-0">
            ...
            <motion.g initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
              {/* Client glow effect */}
              <circle cx="80" cy="180" r="25" fill="url(#clientGlow)" opacity="0.3" />
              <circle cx="80" cy="180" r="18" fill="#000000" stroke="#dc2626" strokeWidth="3" />
              <text x="80" y="186" textAnchor="middle" fontSize="16" fill="#dc2626" fontWeight="bold">⬢</text>
              <text x="80" y="210" textAnchor="middle" fontSize="14" fill="#e2e8f0" fontWeight="600">
                Client
              </text>
            </motion.g>
            ...
            {hops.map((hop, index) => {
              ...
              const prevPos = index === 0 ? { x: 80, y: 180 } : getServerPosition(index - 1);
              ...
              return (
                <motion.g key={`server-${index}`}>
                  ...
                  <motion.line
                    ...
                    transition={{ delay: index * 0.8, duration: 0.8, ease: "easeInOut" }}
                  />
            ...
            <AnimatePresence>
              {finalIPs.map((ip, index) => {
                const pos = getIPPosition(index, finalIPs.length);
                const lastHopPos = getServerPosition(hops.length - 1);
            ...
            <AnimatePresence>
              {packets.map((packet) => {
                const startPos = { x: 80, y: 180 };
                const endPos = getServerPosition(packet.step);
          ...
          {hoveredNode !== null && hops[hoveredNode] && (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="w-3 h-3 rounded-full" style={{ backgroundColor: getRoleColor(detectHopLabel(hops[hoveredNode].server, hops[hoveredNode])) }}></div>
                <span className="text-white font-semibold">
                  {detectHopLabel(hops[hoveredNode].server, hops[hoveredNode]).toUpperCase()} Server
                </span>
              </div>
              <div className="space-y-1 text-sm">
                <div className="text-gray-300 font-mono">{hops[hoveredNode].server}</div>
                <div className="text-gray-400">Response: {hops[hoveredNode].rtt_ms}ms</div>
                <div className="text-gray-400">
                  {hops[hoveredNode].answer?.length > 0 ? "Resolved" : "Referred"}
                </div>
              </div>
            </motion.div>
          )}
  ...
  function SummaryDashboard({ data }) {
    ...
    const chartData = useMemo(() => 
      hops.map(h => ({ step: h.step, rtt: h.rtt_ms ?? 0 })), [hops]
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <div className="space-y-2">
            <div>
              <div className="text-2xl font-bold text-foreground">{summary.total_ms}ms</div>
              <div className="text-xs text-muted-foreground">Total time</div>
            </div>
            <div className="flex gap-4">
              <div>
                <div className="text-lg font-semibold text-primary">{summary.hops}</div>
                <div className="text-xs text-muted-foreground">Hops</div>
              </div>
              {summary.cache_saved_ms > 0 && (
                <div>
                  <div className="text-lg font-semibold text-primary">{summary.cache_saved_ms}ms</div>
          ...
          <div className="h-32">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
                ...
                <RTooltip contentStyle={{ backgroundColor: "#0a0a0a", borderColor: "#262626", color: "#fff" }}/>
                <Area 
                  type="monotone" 
                  dataKey="rtt" 
                  stroke="#dc2626" 
        ...
        <div className="bg-card border border-border rounded-lg p-4">
          ...
          <div className="space-y-2">
            <div>
              ...
              <div className="font-mono text-sm text-foreground">{data?.query?.name} {data?.query?.type}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground mb-1">IPs ({summary.final_ips?.length || 0})</div>
              <div className="space-y-1">
                {summary.final_ips?.slice(0, 2).map((ip, i) => (
                  <div key={i} className="bg-primary/10 text-primary rounded px-2 py-1 font-mono text-xs">
                    {ip}
                  </div>
                )) || (
                  <div className="text-muted-foreground text-xs">No IPs resolved</div>
                )}
                {summary.final_ips?.length > 2 && (
                  <div className="text-xs text-muted-foreground">+{summary.final_ips.length - 2} more</div>
                )}
              </div>
            </div>
          </div>
  ...
  export default function DnsHopExplorer() {
    ...
    const hops = useMemo(() => (Array.isArray(data?.trace) ? data.trace : []), [data]);
    ...
    async function runQuery() {
      ...
      try {
        const url = `${baseUrl}/resolve?name=${encodeURIComponent(name)}&type=${encodeURIComponent(rtype)}&cache=${useCache ? "on" : "off"}`;
        ...
        setError("Unable to connect to DNS resolver. Try using the 'Paste JSON' option instead.");
      } finally {
        setLoading(false);
      }
    }
...
>
  <div className="mx-auto max-w-5xl text-center">
    ...
    <p className="mx-auto mt-3 max-w-2xl text-pretty text-sm text-white/60">
      Visualize how a domain is resolved through the DNS hierarchy. Inspect each hop, timing, and the final IPs.
    </p>
        ...
        {error}
              ...
              {data && narrationLines.length > 0 && (
                <div className="mt-6 rounded-lg border border-gray-800 bg-black/60 backdrop-blur-sm">
                  <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-white">Resolution Steps</span>
                      <span className="text-xs text-gray-400">({Math.min(animStep, narrationLines.length)}/{narrationLines.length})</span>
                    </div>
                    <button 
                  ...
                  <div className="max-h-48 overflow-auto">
                    {narrationLines.map((line, i) => {
                      const isCurrentStep = i === Math.min(animStep - 1, narrationLines.length - 1);
                    ...
                    <div className="px-4 py-3 border-t border-gray-800 bg-gray-900/30">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-300">
                          Resolved to {animResolvedIPs.length} IP{animResolvedIPs.length > 1 ? 's' : ''}
                        </span>
                        <div className="flex gap-1">
                          {animResolvedIPs.slice(0, 2).map((ip, index) => (
                            <span key={index} className="text-xs font-mono bg-gray-800 text-gray-300 px-2 py-1 rounded">
                              {ip}
                            </span>
                          ))}
                          {animResolvedIPs.length > 2 && (
                            <span className="text-xs text-gray-500">+{animResolvedIPs.length - 2}</span>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left text-muted-foreground pb-2 font-medium">Step</th>
                      <th className="text-left text-muted-foreground pb-2 font-medium">Server</th>
                      <th className="text-left text-muted-foreground pb-2 font-medium">Type</th>
                      <th className="text-left text-muted-foreground pb-2 font-medium">RTT</th>
                      <th className="text-left text-muted-foreground pb-2 font-medium">Status</th>
                  ...
                  <tbody>
                    {hops.map((hop, index) => {
                      ...
                      return (
                        ...
                        >
                          <td className="py-2 text-foreground font-medium">{hop.step}</td>
                          <td className="py-2 font-mono text-muted-foreground">{hop.server}</td>
                          <td className="py-2">
                          ...
                          <td className="py-2">
                            {hop.answer?.length > 0 ? (
                              <span className="text-primary text-xs">✓ Answer</span>
                            ) : (
          ...
          {!data && (
            ...
            >
              ...
              <p className="text-sm">Run a query or click <span className="font-medium text-primary">Sample</span> to see the DNS resolution flow.</p>
            </motion.div>
          )}
            ...
            <div className="fixed inset-0 bg-black/90 backdrop-blur z-50 flex items-center justify-center p-4">
              <div className="bg-black border border-gray-700 rounded-lg max-w-5xl w-full max-h-[90vh] overflow-hidden">
                <div className="flex items-center justify-between p-6 border-b border-gray-700">
                  <h2 className="text-2xl font-bold text-white">Complete DNS Guide</h2>
                  <button 
                ...
                <div className="p-6 space-y-8 overflow-y-auto max-h-[75vh]">
                  ...
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">What is DNS Resolution?</h3>
                    <div className="text-gray-300 space-y-3">
                      <p>DNS (Domain Name System) is like the internet's phone book. When you type "google.com" in your browser, DNS translates that human-readable name into an IP address like "172.217.19.36" that computers use to communicate.</p>
                      <p><strong className="text-white">Why is this important?</strong> Without DNS, you'd have to memorize IP addresses for every website you visit. DNS makes the internet user-friendly and enables websites to change servers without affecting users.</p>
                    </div>
                  </section>
  
                  {/* When DNS Happens */}
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">When Does DNS Resolution Occur?</h3>
                    <div className="text-gray-300 space-y-2">
                      <p><strong className="text-white">Every time you:</strong></p>
                      ...
                      <p className="mt-3">DNS lookups happen behind the scenes, usually taking 20-100 milliseconds. Your device caches results to speed up future requests.</p>
                    </div>
                  </section>
  
                  {/* Record Types */}
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">DNS Record Types Explained</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                          <h4 className="font-bold text-white mb-2">A Record (IPv4)</h4>
                          <p className="text-gray-300 text-sm mb-2">Maps domain to IPv4 address</p>
                          <div className="bg-black p-2 rounded font-mono text-xs text-green-400">
                            google.com → 172.217.19.36
                          </div>
                        </div>
                        <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                          <h4 className="font-bold text-white mb-2">AAAA Record (IPv6)</h4>
                          <p className="text-gray-300 text-sm mb-2">Maps domain to IPv6 address</p>
                          <div className="bg-black p-2 rounded font-mono text-xs text-green-400">
                      ...
                      <div className="space-y-4">
                        <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                          <h4 className="font-bold text-white mb-2">CNAME Record</h4>
                          <p className="text-gray-300 text-sm mb-2">Creates an alias pointing to another domain</p>
                        ...
                        <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                          ...
                          <p className="text-gray-300 text-sm mb-2">Specifies authoritative nameservers</p>
                  ...
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">How DNS Resolution Works</h3>
                    <div className="space-y-4">
                      <div className="bg-gray-900/30 p-4 rounded-lg border border-gray-800">
                        <h4 className="font-bold text-white mb-3">Step-by-Step Process:</h4>
                        <div className="space-y-3">
                          <div className="flex items-start gap-3">
                            <span className="bg-red-500 text-white text-xs px-2 py-1 rounded font-bold">1</span>
                            <div>
                              <p className="text-white font-medium">Browser checks local cache</p>
                              <p className="text-gray-400 text-sm">If domain was recently visited, use cached IP</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            <span className="bg-red-500 text-white text-xs px-2 py-1 rounded font-bold">2</span>
                            <div>
                              <p className="text-white font-medium">Query recursive resolver</p>
                              <p className="text-gray-400 text-sm">Your ISP's DNS server or public DNS like 8.8.8.8</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            <span className="bg-red-500 text-white text-xs px-2 py-1 rounded font-bold">3</span>
                            <div>
                              <p className="text-white font-medium">Root server query</p>
                              <p className="text-gray-400 text-sm">Asks root server: "Who handles .com domains?"</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            <span className="bg-red-500 text-white text-xs px-2 py-1 rounded font-bold">4</span>
                            <div>
                              <p className="text-white font-medium">TLD server query</p>
                              <p className="text-gray-400 text-sm">Asks .com server: "Who handles example.com?"</p>
                            </div>
                          </div>
                          <div className="flex items-start gap-3">
                            <span className="bg-red-500 text-white text-xs px-2 py-1 rounded font-bold">5</span>
                            <div>
                              <p className="text-white font-medium">Authoritative server query</p>
                              <p className="text-gray-400 text-sm">Gets final answer with IP address</p>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {/* Example Walkthrough */}
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">Example Walkthrough</h3>
                    <div className="bg-gray-900/30 p-4 rounded-lg border border-gray-800">
                      <h4 className="font-bold text-white mb-3">Looking up "www.google.com":</h4>
                      <div className="space-y-2 font-mono text-sm">
                        <div className="text-gray-400">Query: www.google.com A?</div>
                        <div className="text-red-400">Root Server: "Ask .com TLD server at 192.5.6.30"</div>
                        <div className="text-purple-400">TLD Server: "Ask Google's nameserver at 216.239.32.10"</div>
                        <div className="text-green-400">Authoritative: "www.google.com is 172.217.19.36"</div>
                        <div className="text-white">Result: Your browser connects to 172.217.19.36</div>
                      </div>
                    </div>
                  </section>
                  ...
                  {/* Using This Tool */}
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">How to Use This Tool</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                  ...
                  {/* Common Issues */}
                  <section className="space-y-4">
                    <h3 className="text-xl font-bold text-red-400">Common DNS Issues</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                        <h4 className="font-bold text-white mb-2">NXDOMAIN</h4>
                        <p className="text-gray-300 text-sm">Domain doesn't exist or is misspelled</p>
                      </div>
                      <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                        <h4 className="font-bold text-white mb-2">Timeout</h4>
                        <p className="text-gray-300 text-sm">DNS server not responding</p>
                      </div>
                      <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                        <h4 className="font-bold text-white mb-2">High Latency</h4>
                        <p className="text-gray-300 text-sm">Slow DNS servers or network issues</p>
                      </div>
                      <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-800">
                        <h4 className="font-bold text-white mb-2">Cache Issues</h4>
                        <p className="text-gray-300 text-sm">Stale records or propagation delays</p>
                      </div>
                    </div>
                  </section>
          ...
          {showLearnMore && (
            <div className="fixed inset-0 bg-black/80 backdrop-blur z-50 flex items-center justify-center p-4">
              <div className="bg-black border border-gray-700 rounded-lg max-w-4xl w-full max-h-[80vh] overflow-hidden">
                <div className="flex items-center justify-between p-6 border-b border-gray-700">
                  <h2 className="text-xl font-semibold text-white">DNS Resolution Process</h2>
                  <button 
                ...
                <div className="p-6 space-y-6 overflow-y-auto max-h-[60vh]">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <h3 className="text-lg font-medium text-white">How DNS Resolution Works</h3>
                      <div className="space-y-3 text-sm text-gray-300">
                        <p>DNS resolution is a hierarchical process that translates human-readable domain names into IP addresses that computers use to communicate.</p>
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            <span className="text-red-500 font-bold">◉</span>
                            <span><strong>Root Servers:</strong> Top-level authority that knows TLD server locations</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-purple-500 font-bold">⬣</span>
                            <span><strong>TLD Servers:</strong> Manage top-level domains (.com, .org, etc.)</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-green-500 font-bold">◆</span>
                            <span><strong>Authoritative:</strong> Final authority with actual DNS records</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-yellow-500 font-bold">▣</span>
                            <span><strong>Cache:</strong> Stored results from previous queries</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3 text-sm text-gray-300">
                        ...
                        <div>
                          <strong className="text-white">Response Sections:</strong>
                          <ul className="mt-1 space-y-1 ml-4">
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <h3 className="text-lg font-medium text-white mb-3">Resolution Flow</h3>
                    <div className="text-sm text-gray-300 space-y-2">
                      <p>1. <strong>Recursive Query:</strong> Client asks resolver for domain IP</p>
                      <p>2. <strong>Root Query:</strong> Resolver queries root server for TLD information</p>
                      <p>3. <strong>TLD Query:</strong> Resolver queries TLD server for authoritative nameservers</p>
                      <p>4. <strong>Authoritative Query:</strong> Resolver queries authoritative server for final answer</p>
                      <p>5. <strong>Response:</strong> IP address returned to client and cached for future use</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {showJsonInput && (
            <div className="fixed inset-0 bg-background/80 backdrop-blur z-50 flex items-center justify-center p-4">
              <div className="bg-card border border-border rounded-lg max-w-2xl w-full max-h-[80vh]">
                <div className="flex items-center justify-between p-4 border-b border-border">
                  <h2 className="text-lg font-semibold text-foreground">Paste DNS Resolution JSON</h2>
                  <button 
